...
export function setMainState(state) {
  ...
  const readerProfEl = document.getElementById('reader-profile');
  ...
  if(readerProfEl) readerProfEl.classList.remove('visible');
  ...
  else if(state === 'reader-profile') { if(readerProfEl) readerProfEl.classList.add('visible'); }
}
...
window.addEventListener('popstate', () => {
  ...
  if(navStack.length > 0) {
    ...
    } else if(curr.type === 'reader-profile') {
      window.closeReaderProfile();
